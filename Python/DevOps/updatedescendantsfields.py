...
pat = os.getenv('pat')
print(pat)
parent_work_item_id = 21858  # ID of the parent work item
field_to_set = "Tags"
field_value = "Dummy"
...
def update_work_item(work_item_id, field_name, value):
    ...
    if response.status_code == 200:
        ...
        logging.error(f"{response.status_code} - Failed to update Work Item ID {work_item_id}: {response.text}")
        print(f"{response.status_code} - Failed to update Work Item ID {work_item_id}: {response.text}")
